...
class DockerhubPipeline(object):
    ...
    def _do_upinsert(self, conn, item, spider):
    	conn.execute("insert into officialimage(name, url) values (%s, %s)", (item['name'], item['link']))
    	print "insert into officialimage(name, url) values (%s, %s)", (item['name'], item['link'])
        # linkmd5id = self._get_linkmd5id(item)
        #print linkmd5id
        # now = datetime.utcnow().replace(microsecond=0).isoformat(' ')
        # conn.execute("""
        #         select 1 from officialimage where linkmd5id = %s
        # """, (linkmd5id, ))
        # ret = conn.fetchone()
